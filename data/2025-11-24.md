...
TL;DR: Cognitive BASIC是一种基于BASIC风格的最小化提示语言和模型内解释器，将大语言模型的推理过程结构化为显式的逐步执行轨迹。


<details>
  <summary>Details</summary>
Motivation: 受复古BASIC简单性的启发，重新利用编号行和简单命令作为可解释的认知控制层，使现代LLM能够可靠地模拟这类短程序，实现模型内部的透明多步推理。
...
TL;DR: 提出了一个系统化的基准测试修订框架，通过分析响应模式的统计特征来识别可能无效的问题，结合专家审查和LLM初步筛选，提高基准测试的可靠性。
...
Method: 基于AI评估中常用的核心假设（均值足以总结模型性能），构建单维潜在结构模型，通过统计异常检测识别问题项，并引入LLM法官进行初步筛选。

Result: 在9个广泛使用的基准测试中，该方法指导专家审查识别问题问题的精确度高达84%，显著减少了人工工作量。

Conclusion: 该框架为系统化基准测试修订提供了高效且可扩展的解决方案，能够有效识别和修正基准测试中的无效问题。
...
### [4] [Agentifying Agentic AI](https://arxiv.org/abs/2511.17332)
...
Result: 提出了一个连接形式理论和实践自主性的智能视角，为构建不仅能力强、灵活，而且透明、合作和可问责的智能系统奠定了基础。
...
### [5] [Hybrid Differential Reward: Combining Temporal Difference and Action Gradients for Efficient Multi-Agent Reinforcement Learning in Cooperative Driving](https://arxiv.org/abs/2511.16916)
*Ye Han,Lijun Zhang,Dejian Meng,Zhuang Zhang*

Main category: cs.AI

TL;DR: 针对多车辆协同驾驶中传统奖励函数存在的奖励差异消失问题，本文提出了一种混合差分奖励(HDR)机制，通过整合时间差分奖励和动作梯度奖励，显著提高了策略梯度的信噪比和算法收敛性能。
...
Method: 提出混合差分奖励(HDR)机制，包含两个互补组件：基于全局势函数的时间差分奖励(TRD)和直接测量动作边际效用的动作梯度奖励(ARG)。将协同驾驶问题建模为具有时变智能体集的多智能体部分可观测马尔可夫博弈。

Result: 使用在线规划(MCTS)和多智能体强化学习(QMIX、MAPPO、MADDPG)算法进行的广泛实验表明，HDR机制显著提高了收敛速度和策略稳定性。
...
<details>
  <summary>Details</summary>
Motivation: 现有工具增强智能体在扩展测试时计算时，单纯增加工具调用预算无法提升性能，因为智能体缺乏"预算意识"，会快速达到性能瓶颈。

Method: 提出预算跟踪器插件提供持续预算意识，并开发BATS框架，利用预算意识动态调整规划和验证策略，决定是深入挖掘有希望的线索还是转向新路径。

Result: 预算感知方法产生了更有利的扩展曲线，并推动了成本-性能帕累托前沿，提供了对工具增强智能体扩展的更透明和原则性理解。

Conclusion: 预算感知方法能有效改善工具增强智能体的扩展性能，为理解此类智能体的扩展提供了实证见解。
...
### [7] [DAPS++: Rethinking Diffusion Inverse Problems with Decoupled Posterior Annealing](https://arxiv.org/abs/2511.17038)
*Hao Chen,Renzheng Zhang,Scott S. Howard*

Main category: cs.AI

TL;DR: 论文重新解释了扩散模型在逆问题求解中的作用，提出DAPS++方法将扩散阶段与数据驱动优化解耦，通过更直接地利用似然项指导推理，在保持数值稳定性的同时提高计算效率。


<details>
  <summary>Details</summary>
Motivation: 传统贝叶斯视角下基于分数的扩散方法在解决逆问题时，先验提供的指导有限，重建主要由测量一致性项驱动，导致推理过程与扩散动力学脱节。需要澄清这种结构并改进方法。

Method: 将扩散重新解释为期望最大化框架中的初始化阶段，提出DAPS++方法，将扩散阶段与数据驱动优化完全解耦，让似然项更直接地指导推理过程。

Result: DAPS++需要更少的函数评估次数和测量优化步骤，在多种图像恢复任务中实现了高计算效率和鲁棒的重建性能。

Conclusion: DAPS++通过解耦扩散与数据优化，提供了对统一扩散轨迹有效性的解释，同时显著提高了逆问题求解的计算效率。
...
### [8] [MIR: Efficient Exploration in Episodic Multi-Agent Reinforcement Learning via Mutual Intrinsic Reward](https://arxiv.org/abs/2511.17165)
*Kesheng Chen,Wenjian Luo,Bang Zhang,Zeping Yin,Zipeng Ye*

Main category: cs.AI

TL;DR: 本文提出了一种名为MIR的互惠内在奖励方法，用于解决多智能体强化学习中稀疏奖励问题，特别是在回合奖励场景下。该方法通过激励智能体探索影响队友的行为，有效促进团队探索并提升算法性能。


<details>
  <summary>Details</summary>
Motivation: 多智能体强化学习中的回合奖励面临两大挑战：联合动作轨迹的指数级稀疏性，以及现有方法未能充分考虑影响团队状态的联合动作。需要一种有效的方法来应对这些稀疏奖励问题。

Method: 提出了MIR（互惠内在奖励）方法，这是一种简单但有效的增强策略。MIR激励个体智能体探索能够影响队友的动作，当与原始策略结合时，能够有效刺激团队探索。

Result: 在扩展的MiniGrid-MA环境中进行实验验证，与最先进方法相比，MIR方法表现出优越的性能，特别是在稀疏奖励场景下。

Conclusion: MIR方法能够有效解决多智能体强化学习中的稀疏奖励问题，通过促进智能体间的相互影响和团队探索，显著提升了算法性能。
...
TL;DR: 本文系统评估了使用合成和off-policy数据训练LLM行为探测器的泛化能力，发现在缺乏on-policy数据时，使用同域off-policy数据比不同域on-policy数据更可靠，但域偏移仍会导致性能显著下降。


<details>
  <summary>Details</summary>
Motivation: 由于许多LLM行为的自然样本稀少，研究者不得不依赖合成或off-policy的LLM响应来训练探测器，但这种方法对探测器泛化能力的影响尚未得到系统评估。

Method: 在8种不同LLM行为上测试线性和注意力探测器，评估不同响应生成策略对探测器性能的影响，并分析从off-policy到on-policy数据的泛化能力。

Result: 响应生成策略显著影响探测器性能，成功从off-policy数据泛化到激励模型产生目标行为的测试集，能预测成功的on-policy泛化。Deception和Sandbagging探测器在真实监控场景中可能无法从off-policy泛化到on-policy数据。

Conclusion: 训练数据域的偏移比响应生成策略的变化导致更大的性能下降，强调需要能更好处理LLM监控中分布偏移的方法。
...
Result: 相比通用协作感知方法，安全关键对象平均精度损失小于1%，仅使用20%通信带宽；相比现有选择性协作感知方法，感知性能提升15%

Conclusion: SRA-CP框架有效解决了协作感知中的通信带宽和动态环境适应性问题，在保持安全性的同时显著提升效率
...
### [11] [Optimizing PyTorch Inference with LLM-Based Multi-Agent Systems](https://arxiv.org/abs/2511.16964)
*Kirill Nagaitsev,Luka Grbcic,Samuel Williams,Costin Iancu*

Main category: cs.MA

TL;DR: 本研究提出了一个比较多智能体PyTorch优化系统的逻辑框架，发现利用性强的策略与错误修复智能体结合时表现最佳，性能与优化步骤的粒度相关，最佳实现在H100 GPU上平均实现2.88倍加速。
...
Method: 提出了一个逻辑框架来比较多智能体PyTorch优化系统，评估了不同策略与错误修复智能体的组合效果，以及优化步骤粒度对性能的影响。

Result: 利用性强的策略与错误修复智能体结合时表现最佳，性能与优化步骤的粒度相关，最佳实现在KernelBench基准套件的多样化任务上平均实现2.88倍加速。

Conclusion: 多智能体系统在GPU性能优化中具有显著潜力，特定策略组合和优化粒度控制能有效提升性能，为自动化AI推理系统优化提供了新方向。
...
### [12] [A segment anchoring-based balancing algorithm for agricultural multi-robot task allocation with energy constraints](https://arxiv.org/abs/2511.17076)
...
TL;DR: 提出SABA算法解决多机器人系统在能量约束下的任务分配问题，通过分段锚定和平衡机制处理充电决策带来的调度中断，在收敛性和多样性方面优于现有算法。


<details>
  <summary>Details</summary>
Motivation: 多机器人系统在农业等劳动密集型产业中面临效率挑战，特别是在智能农业场景中，电动机器人的多行程收割调度需要考虑能量约束、负载约束和多个冲突目标，传统优化方法难以有效处理充电决策带来的复杂级联效应。

Method: 提出分段锚定平衡算法(SABA)，包含两个协同机制：顺序锚定和平衡机制利用充电决策作为锚点系统重构中断路径；基于比例分割的再平衡机制负责精细调整最终解的时间跨度。

Result: 在真实案例和基准实例上的实验表明，SABA在解收敛性和多样性方面全面优于6种最先进算法。
...
TL;DR: MicroEP是一种新颖的并行化策略，通过跨GPU的高效token调度实现MoE系统中的细粒度负载均衡。MicroMoE系统基于此策略，相比最先进系统将端到端训练吞吐量提升高达47.6%。


<details>
  <summary>Details</summary>
Motivation: MoE的动态特性导致专家间负载不均衡，严重影响训练效率。现有解决方案要么牺牲模型精度，要么引入额外系统开销，无法实现细粒度负载均衡。

Method: 提出MicroEP并行化策略，通过高效的token调度在每微批次中实现最优负载均衡。构建MicroMoE分布式MoE训练系统，集成MicroEP的负载均衡能力。

Result: 实验结果显示，MicroMoE相比最先进系统将端到端训练吞吐量提升高达47.6%，几乎持续实现GPU间的最优负载均衡。

Conclusion: MicroEP和MicroMoE成功解决了MoE系统中的负载均衡问题，显著提升了训练效率，为大规模深度学习模型的高效训练提供了有效解决方案。
